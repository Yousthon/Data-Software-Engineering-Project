...
public class DateFrancaise extends GregorianCalendar {
	...
	public static void setDate(DateFrancaise date) throws DateFrancaiseException {
		dateSimulee = new DateFrancaise(date.get(Calendar.DAY_OF_MONTH), date.get(Calendar.MONTH) + 1,
	...
	/**
	 * Pour obtenir un nouvel objet de type DateFrancaise à la valeur de la date
	 * simulée ou réelle
	 * 
	...
	public DateFrancaise(int jour, int mois, int an, int heure, int minute) throws DateFrancaiseException {
		...
		if (get(Calendar.YEAR) != an)
			throw new DateFrancaiseException();
		if (get(Calendar.MONTH) != (mois - 1))
			throw new DateFrancaiseException();
		if (get(Calendar.DAY_OF_MONTH) != jour)
			throw new DateFrancaiseException();
		if (get(Calendar.HOUR_OF_DAY) != heure)
			throw new DateFrancaiseException();
	...
	/**
	 * Construction d'une DateFrancaise clone de la DateFrancaise passée en
	 * paramètre
	 * 
	...
	/**
	 * Construction d'une DateFrancaise clone de la GregorianCalendar passée en
	 * paramètre
	 * 
	 * @param date : la date à cloner
	 */
	...
	/**
	 * renvoie true si la DateFrancaise est dans le passé par rapport à la
	 * DateFrancaise du moment simulé
	 */
